...
#define INF(format, ...) Util::Logger::getInstance()->info(SPDLOG_FMT_RUNTIME(std::string("[{}:{}] ") + (format)), __FILE__, __LINE__, ##__VA_ARGS__)
#define WRN(format, ...) Util::Logger::getInstance()->warn(SPDLOG_FMT_RUNTIME(std::string("[{}:{}] ") + (format)), __FILE__, __LINE__, ##__VA_ARGS__)
