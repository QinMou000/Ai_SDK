...
class IHttpTransport {
   public:
    ...
    virtual HttpResponse postJson(const std::string& url, const nlohmann::json& body, const HttpHeaders& headers,
                                  int timeout_ms) const = 0;
    // 流式实现必须保持 callback 异常的原始传播语义。
    virtual HttpResponse postJsonStream(const std::string& url, const nlohmann::json& body, const HttpHeaders& headers,
                                        int timeout_ms, HttpStreamCallback callback) const = 0;
};
...
class HttpClient {
   public:
    ...
    HttpResponse postJson(const std::string& url, const nlohmann::json& body, const HttpHeaders& headers,
                          int timeout_ms, TraceSession& trace_session, const std::string& parent_step_id) const;
