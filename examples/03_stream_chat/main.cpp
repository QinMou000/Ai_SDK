...
int main(int argc, char** argv) {
    try {
        ...
        client.streamChat(request, [](const aiSDK::StreamEvent& event) {
            if(event.type == aiSDK::StreamEventType::Delta) {
                std::cout << event.delta << std::flush;
            } else if(event.type == aiSDK::StreamEventType::Error) {
                std::cerr << "\n流式请求出错: " << event.error_message << std::endl;
            }
        });
        ...
        std::cerr << "DeepSeek 流式示例执行失败: " << exception.what() << std::endl;
        return 1;
    }
}
