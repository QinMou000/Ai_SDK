...
- `include/mcp/`：独立 MCP 配置、值类型、Client、Transport 窄接口与 Tool Adapter；不得暴露 cpr、libcurl 或平台句柄。
- `include/agent/`：目前只有头文件定义或预留抽象，没有对应 `src/` 实现。
...
- `src/mcp/`：MCP 生命周期、JSON-RPC、stdio、Streamable HTTP 和 Tool Adapter；`detail/` 保存未导出的协议、SSE、测试 HTTP 工厂与进程抽象，`process/` 保存 Windows/POSIX 条件源。
- `src/AIClient.cpp`：SDK 门面，负责 Provider 选择、工具注册入口与对外聊天接口。
- `examples/`：按场景拆分的示例入口，展示真实调用方式而不是测试替身。
- `tests/`：GTest 测试；当前分为 `core`、`provider`、`http`、`tool`、`trace`、`mcp`、`smoke`。
...
- MCP 协议单元、真实 stdio、真实 HTTP/TLS 验证统一放在 `tests/mcp/`；平台 TLS 夹具只放 `tests/mcp/helpers/`。
...
- `include/agent/` 目前没有 `src/` 实现，新增能力前先确认是否真的需要落地，而不是沿着预留目录盲目扩展。
- `include/mcp/` 已有独立 `ai_sdk_mcp` 目标；Skill Loader 仍属于未来 Agent 层，不得因名称相近放入 MCP 协议目录。
...
- 新增 MCP 公开合同：放 `include/mcp/`；协议与传输实现放 `src/mcp/`，平台进程源放 `src/mcp/process/`，确定性及真实集成测试放 `tests/mcp/`。
